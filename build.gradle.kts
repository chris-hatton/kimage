...
val publishVersion : String = run {
    ...
    val branchName : String? = System.getenv("CI_COMMIT_REF_NAME")
    val buildId    : String? = System.getenv("CI_PIPELINE_IID")

    when(branchName) {
        "release" -> "$majorVersion.$minorVersion.$buildId"
        "master"  -> "$majorVersion.$minorVersion.0-SNAPSHOT"
        else      -> "$majorVersion.$minorVersion.0-SNAPSHOT"
    }
}
